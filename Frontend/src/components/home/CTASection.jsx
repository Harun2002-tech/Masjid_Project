  ...
  return (
    <div className="text-center group">
      ...
      <p className="text-5xl md:text-7xl font-bold tracking-tighter text-white mb-3 tabular-nums">
        <CountUp end={numericValue || 0} duration={3} enableScrollSpy scrollSpyOnce />
        {hasPlus && <span className="text-gold ml-1 text-4xl">+</span>}
      </p>
      <p className={`text-[10px] font-black uppercase tracking-[0.4em] text-white/30 group-hover:text-gold transition-colors ${bodyFont}`}>
        {label}
      </p>
    </div>
  );
...
export default function CTASection() {
    ...
    const fetchStats = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          localStorage.setItem("ruhama_stats", JSON.stringify(response.data.data));
        }
      } catch (error) {
  ...
  const titleFont = language === "am" ? "font-amharic" : language === "ar" ? "font-serif" : "font-serif italic";
  const bodyFont = language === "am" ? "font-amharic" : "";

  return (
    <section className={`py-40 relative overflow-hidden bg-[#05080f] ${bodyFont}`} dir={dir}>
      {/* Cinematic Lighting */}
      ...
      <div className="mx-auto max-w-7xl px-6 relative z-10">
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className={`glass p-12 md:p-16 rounded-[4rem] border border-white/5 hover:border-gold/20 transition-all duration-700 shadow-2xl relative overflow-hidden group ${dir === "rtl" ? "text-right" : "text-left"}`}
          >
            <div className={`absolute top-0 ${dir === "rtl" ? "left-0" : "right-0"} p-10 opacity-5 group-hover:opacity-10 transition-opacity`}>
            ...
            <h3 className={`text-4xl md:text-7xl font-bold text-white mb-10 leading-[1.1] ${titleFont}`}>
              {language === "am" ? "ጉዟችሁን ይጀምሩ" : language === "ar" ? "ابدأ رحلتك" : "Begin Journey"}
            ...
            <Link to="/courses">
              <button className="btn-gold h-20 px-14 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] flex items-center gap-6 group shadow-gold-glow">
                {language === "am" ? "አሁን ይመዝገቡ" : language === "ar" ? "سجل الآن" : "Apply Now"}
                {dir === "rtl" ? (
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className={`glass p-12 md:p-16 rounded-[4rem] border border-white/5 hover:border-red/20 transition-all duration-700 shadow-2xl relative overflow-hidden group ${dir === "rtl" ? "text-right" : "text-left"}`}
          >
            <div className={`absolute top-0 ${dir === "rtl" ? "left-0" : "right-0"} p-10 opacity-5 group-hover:opacity-10 transition-opacity`}>
            ...
            <h3 className={`text-4xl md:text-7xl font-bold text-white mb-10 leading-[1.1] ${titleFont}`}>
              {language === "am" ? "ተልዕኳችንን ይደግፉ" : language === "ar" ? "ادعم رسالتنا" : "Support Mission"}
            ...
            <Link to="/donations">
              <button className="h-20 px-14 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] border border-white/10 text-white hover:bg-white hover:text-black transition-all flex items-center gap-6 group">
                {language === "am" ? "አሁኑኑ ይለግሱ" : language === "ar" ? "تبرع الآن" : "Donate Now"}
                <Heart size={20} className="fill-current group-hover:scale-125 transition-transform duration-500" />
              </button>
            </Link>
          </motion.div>
        </div>
        ...
        >
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-16 md:gap-24">
            <Stat
              ...
              label={language === "am" ? "ተማሪዎች" : language === "ar" ? "الطلاب" : "Students"}
              icon={<Users />}
              bodyFont={bodyFont}
              dir={dir}
            />
            <Stat
              ...
              label={language === "am" ? "ዓመታት" : language === "ar" ? "سنوات" : "Years"}
              icon={<Sparkles />}
              bodyFont={bodyFont}
              dir={dir}
            />
            <Stat
              ...
              label={language === "am" ? "ሊቃውንት" : language === "ar" ? "العلماء" : "Scholars"}
              icon={<ShieldCheck />}
              bodyFont={bodyFont}
              dir={dir}
            />
            <Stat
              ...
              label={language === "am" ? "ኮርሶች" : language === "ar" ? "الدورات" : "Courses"}
              icon={<BookOpen />}
              bodyFont={bodyFont}
              dir={dir}
            />
